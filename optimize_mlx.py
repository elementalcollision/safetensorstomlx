...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Path to write the output optimized MLX file (default: input dir with suffix)"
    )
    ...
    parser.add_argument(
        ...
        help="Analyze model structure to identify tensor distribution and MoE components"
    )
...
def analyze_model_structure(input_file: Path, verbose: bool = False):
    """
    Analyze the structure of a MLX model to understand tensor distribution and identify
    MoE components.
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Could not load as safetensors, trying generic load: {e}")
            try:
        ...
        for name, tensor in weights.items():
            ...
            if name.endswith("output.weight") or name.endswith(
                    "lm_head.weight"):
                results["output_tensor"] = name

            # Check for token embedding tensor
            if name.endswith("token_embeddings.weight") or name.endswith(
                    "embed_tokens.weight"):
                results["token_embedding_tensor"] = name

            if verbose:
                logger.debug(
                    f"Tensor: {name}, Shape: {tensor.shape}, Type: {tensor.dtype}, "
                    f"Size: {tensor_size/1024/1024:.2f} MB")

        # Combine expert and router tensors into moe_tensors
        results["moe_tensors"] = results["expert_tensors"] + \
            results["router_tensors"]
        ...
        if results["has_moe"]:
            logger.info(
                f"MoE expert tensors: {len(results['expert_tensors'])}")
            logger.info(
                f"MoE router tensors: {len(results['router_tensors'])}")
        ...
        with open(analysis_file, "w") as f:
            ...
            serializable_results = {
                k: v if not isinstance(
                    v,
                    defaultdict) else dict(v) for k,
                v in results.items()}
            # Convert tensor shapes dict values from lists to counts for
            # readability
            serializable_results["tensor_shapes"] = {
                k: len(v) for k, v in serializable_results["tensor_shapes"].items()}
...
def optimize_mlx_model(args):
    ...
    if args.analyze_model:
        ...
        try:
            ...
            weights = model_dict.get("weights", {})
            has_moe = any(
                "expert" in name or "experts" in name or "router" in name or "gate" in name
    ...
    try:
        ...
        metadata["optimization_date"] = str(
            import_datetime().now().isoformat())
        ...
        for name, tensor in weights.items():
            ...
            if args.leave_output_tensor and (
                    name.endswith("output.weight") or name.endswith("lm_head.weight")):
            ...
            elif (args.token_embedding_type
                  and (name.endswith("token_embeddings.weight")
                       or name.endswith("embed_tokens.weight"))):
                target_dtype = args.token_embedding_type
                logger.debug(
                    f"Setting token embedding tensor {name} to {target_dtype}")

            # Check if this is a MoE expert tensor
            elif (has_moe and ("expert" in name or "experts" in name)
                  and args.moe_expert_optimization != "same"):
                target_dtype = args.moe_expert_optimization
                logger.debug(
                    f"Setting MoE expert tensor {name} to {target_dtype}")

            # Check if this is a MoE router tensor
            elif (has_moe and ("router" in name or "gate" in name)
                  and args.moe_router_optimization != "same"):
                target_dtype = args.moe_router_optimization
                logger.debug(
                    f"Setting MoE router tensor {name} to {target_dtype}")

            # Default optimization type
            else:
                target_dtype = args.type

            # Skip optimization if tensor is already in the target format and
            # reoptimization is not allowed
            if not args.allow_reoptimize and str(tensor.dtype) == target_dtype:
                logger.debug(
                    f"Tensor {name} already in {target_dtype} format, skipping")
            ...
            if target_dtype == "fp16":
                ...
                # MLX doesn't support bf16, so we'll use fp16 as the closest
                # alternative
                logger.warning(
                    "MLX doesn't support bfloat16, using float16 instead")
                optimized_weights[name] = tensor.astype(mx.float16)
            else:  # fp32
...
def main():
    ...
    if args.analyze_model and args.type == "auto":
        ...
        try:
            ...
            if "error" in analysis_results:
                logger.error(
                    f"Error analyzing model: {analysis_results['error']}")
                return 1
            ...
            if has_moe:
                ...
                logger.info(
                    "This model contains Mixture of Experts (MoE) architecture.")
                logger.info("Recommended optimization settings:")
                logger.info(
                    "  1. For better quality: --type fp16 --leave-output-tensor "
                    "--moe-expert-optimization fp32")
                logger.info(
                    "  2. For better size: --type fp16 --moe-expert-optimization fp16")
                logger.info(
                    "  3. For balanced approach: --type fp16 --moe-router-optimization fp32")
            else:
                ...
                logger.info(
                    "  3. For balanced approach: --type fp16 --token-embedding-type fp32")
